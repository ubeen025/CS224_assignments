if __name__ == '__main__':    
    ...
    block_size = 128
    text = open_utf8(args.pretrain_corpus_path).read()
    ...
    if args.function == 'pretrain':
        ...
        finetune_corpus = open_utf8(args.finetune_corpus_path).read()
        ...
        if args.eval_corpus_path is not None:
            ...
            eval_corpus = open_utf8(args.eval_corpus_path).read()
        ...
        correct = 0
        total = 0
        with open_utf8(args.outputs_path, 'w') as fout:
            predictions = []
            for line in tqdm(open_utf8(args.eval_corpus_path)):
                x = line.split('\t')[0]
                x = x + '⁇'
