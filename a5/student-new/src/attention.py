...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        B, T, C = x.size()   #B- batch size, T- sequence length(# of tokens) , C- # of features or the embedding dimension.
...
class SynthesizerAttention(nn.Module):
    ...
    def forward(self, x, layer_past=None):
        ...
        # Calculate synthesized inputs and values for all heads in batch and move head forward to be the batch dim
        z = self.w1(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs) changing the dimension
        # [linear transformation using w1]. change dimension into [B , T, #of attention heads, #Size of each head's output]
        v = self.value(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, T, hs)
        # [linear transformation using value] 

        # Synthesized self-attention; Self-attend: (B, nh, T, hs) x (1, 1, hs, T) -> (B, nh, T, T)
        att = F.relu(z) @ self.w2[:, :T] + self.b2[:T] # (B, nh, T, T)    
        # relu(x)= max(0,x) if x positive, unchanged, if x negative, replaced ny 0.
        att = att.masked_fill(self.mask[:,:,:T,:T] == 0, -1e10) # todo: just use float('-inf') instead?
        att = F.softmax(att, dim=-1)
        att = self.attn_drop(att)
        y = att @ v # (B, nh, T, T) x (B, nh, T, hs) -> (B, nh, T, hs)
